...
## Question 1d)
```{r}
bootstrap_test <- function(n = 200, b_count = 100, mu, test = min(data$video)){
  ...
  pl = sum(boot_results < test)/1000
  pr = sum(boot_results > test)/1000
  p_value = 2 * min(pl, pr)
  ...
  # if (p_value > 0.05){main_results[i] = 0}
  # else {main_results[i] = 1}
